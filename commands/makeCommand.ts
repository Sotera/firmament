...
export class MakeCommand extends CommandImpl {
  ...
  private buildTemplate(argv:any) {
    let fullInputPath = this.getJsonConfigFilePath(argv.input);
  ...
  private makeTemplate(argv:any) {
    ...
    let fullOutputPath = this.getJsonConfigFilePath(argv.output);
    var fs = require('fs');
    if (fs.existsSync(fullOutputPath)) {
      var positive = require('positive');
      if (positive("Config file '" + fullOutputPath + "' already exists. Overwrite? [Y/n]", true)) {
        this.writeJsonTemplateFile(fullOutputPath, argv.full);
      } else {
  ...
  private getJsonConfigFilePath(filename) {
  ...
  private processContainerConfigs(containerConfigs) {
    ...
    sortedContainerConfigs = this.containerDependencySort(containerConfigs);
    /*
  ...
  private writeJsonTemplateFile(fullOutputPath:string, writeFullTemplate:boolean) {
